...
クエリ（検索）パラメータ（変数）や、クエリストリングとも言う

サーバーへ送りたいデータを指定するためにURLの最後に追加する文字列です。ページや媒体、広告ごとに違うクエリパラメータを付与することで、どこから流入してきたかを判別できます。

```
https://example.com/lp_01/index.html?waad=aBcDXxXx
```

上記の場合、?から後ろの「?s=gaiq」がクエリパラメータです。
...
上記の「12345」の部分がユーザーIDなどを表す変数となっています。
